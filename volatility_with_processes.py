...
class VolatilityParser(multiprocessing.Process):

    def __init__(self, ticker_name, ticker_path, collector, *args, **kwargs):
    ...
    def volatility_calculation(self):
        ...
        self.collector.put(dict(name=self.ticker_name, vol=self.volatility))
...
class TickerExtractor:
    ...
    def run(self):
        ...
        tickers = [VolatilityParser(ticker_name=name, ticker_path=path, collector=collector)
        ...
        while not collector.empty():
            self.data = collector.get()
            self.result_dict[self.data['name']] = self.data['vol']
        self.report()
